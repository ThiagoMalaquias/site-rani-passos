  ...
  return _createClass(_default, [{
    ...
    value: function configureGroup(fragment) {
      ...
      group.querySelectorAll("select, input").forEach(function (element) {
        if (element.name) {
          element.name = element.name.replace(templatePrefix, "".concat(_this.prefixValue, "[").concat(groupId, "]")).replace("lesson[", "".concat(_this.prefixValue, "["));
        }
        if (element.id) {
    ...
    function adjustNames(fragment) {
      ...
      fragment.querySelectorAll("select, input").forEach(function (element) {
        ...
        if (element.id) {
          element.id = "".concat(element.id, "-").concat(timestamp);
        }
      });
      fragment.querySelectorAll("label").forEach(function (label) {
        var htmlFor = label.getAttribute("for");
        if (htmlFor) label.setAttribute("for", "".concat(htmlFor, "-").concat(timestamp));
      });
    }
  }, {
    key: "removeGroup",
    ...
    value: function renderVideos(group, videos) {
      ...
      var templatePrefix = group.dataset.templatePrefix.replace("lesson[", "".concat(this.prefixValue, "["));
      ...
      videos.forEach(function (video) {
        ...
        var fieldName = _this3.prefixValue.includes("review") ? "".concat(finalPrefix, "[videos][]") : "lesson[lesson_lecture_videos_attributes][][lecture_video_id]";
        input.name = fieldName;
        input.value = video.id;
        input.id = "lecture_video_".concat(video.id, "_").concat(Date.now());
...
/******/ });
//# sourceMappingURL=application-3cd164ec1df647c195c6.js.map